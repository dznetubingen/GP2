...
out[['gp2_oth_case_homalt','gp2_oth_case_het','gp2_case_ac']] = out.GP2_affected_gt.str.split(",",expand=True)
...
out['gp2_oth_case_het']= out['gp2_oth_case_het'].astype(int)-out['fam_case_het'].astype(int)
out['gp2_oth_case_homalt']=out['gp2_oth_case_homalt'].astype(int)-out['fam_case_homalt'].astype(int)

def get_max_str(lst):
    return max(lst, key=len)

tmp=out.join(out.loc[:,get_max_str(out.columns)].str.split(';', expand=True).add_prefix('ann'))
tmp.drop(['GP2_affected_gt','Cases','Controls','gene',get_max_str(out.columns)], axis=1, inplace=True)
...
#columns to drop
allvars= tmp.drop(['gp2_case_ac','fam_ctrl_ac','fam_case_ac', 'VAR_SYNONYMS'],axis=1)

##need to remove amp-pd once it's fixed
allvars.rename(columns = {'Gene':'Ensembl_geneID', '#mode':'mode',
                          'NEAREST':'NEAREST_gene', 
                          'brain_celltype':'brain_celltype_regulatory_element',
                          'amppd_case_homalt':'amppd_case_nhomalt_3359',
                          'amppd_case_het':'amppd_case_nhet_3359'}, inplace = True)

allvars[['SpliceAI_pred_DS_AG','SpliceAI_pred_DS_AL', 'SpliceAI_pred_DS_DG', 'SpliceAI_pred_DS_DL','CADD_PHRED','gnomad_AF','gnomad_popmax_af','gnomad_nhomalt']] = allvars[['SpliceAI_pred_DS_AG','SpliceAI_pred_DS_AL', 'SpliceAI_pred_DS_DG', 'SpliceAI_pred_DS_DL','CADD_PHRED','gnomad_AF','gnomad_popmax_af','gnomad_nhomalt']].apply(pd.to_numeric)

## rearrange columns
## add gene_fullname, mane, regulatroy motif, transcription factor, TSSdistance
column_names = ['mode','chr:pos:ref:alt','family_id','gene','gene_fullname','Ensembl_geneID','BIOTYPE', 'transcript','STRAND','CANONICAL','MANE_SELECT','MANE_PLUS_CLINICAL','TSL','EXON','Codons','Amino_acids','DownstreamProtein', 'ProteinLengthChange','HGVSc','HGVSp','highest_impact','NMD','ClinVar_CLNSIG','ClinVar_CLNDN','Existing_variation','ClinVar_link','OMIM_link','CNCR','CADD_PHRED','gnomAD_pLI', 'gnomAD_oe_lof_CI90','gnomAD_oe_mis_CI90', 'gnomAD_oe_syn_CI90', 'clinvar_gene_description','MOI','gnomad_AF', 'gnomad_popmax_af', 'gnomad_nhomalt', 'gnomad_AC','TOPMed8_AF','gp2_oth_case_homalt', 'gp2_oth_case_het','fam_ctrl_homalt', 'fam_ctrl_het','fam_case_homalt', 'fam_case_het','amppd_case_nhet_3359', 'amppd_case_nhomalt_3359','impact','NEAREST_gene', 'MAX_AF_POPS','LoF','LoF_filter','LoF_flags','LoF_info','SpliceAI_pred_DS_AG', 'SpliceAI_pred_DS_AL', 'SpliceAI_pred_DS_DG', 'SpliceAI_pred_DS_DL', 'SpliceAI_pred_SYMBOL','SpliceRegion', 'existing_InFrame_oORFs', 'existing_OutOfFrame_oORFs', 'existing_uORFs', 'five_prime_UTR_variant_annotation','five_prime_UTR_variant_consequence', 'miRNA', 'MOTIF_NAME', 'MOTIF_POS','HIGH_INF_POS','MOTIF_SCORE_CHANGE' ,'TSSDistance','TRANSCRIPTION_FACTORS','brain_celltype_regulatory_element','DisGeNET','MetaRNN_score', 'Ensembl_transcriptid', 'sample_id', 'genotype(sample,dad,mom)', 'depths(sample,dad,mom)', 'allele_balance(sample,dad,mom)','IMPACT']
...
##maybe do it at gcount?
#thresold = N_ind * 0.1
#allvars = allvars.loc[(allvars.gp2_case_homalt < = thresold) | (allvars.gp2_case_het < = thresold)]
...
AD1_1 = tier1[(tier1['mode'].str.contains('dominant|denovo')) & (tier1['gnomad_popmax_af'] < 0.005)]
