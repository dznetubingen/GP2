...
workflow JointGenotyping {
  ...
  String gatk_docker = select_first([gatk_docker_override, "broadinstitute/gatk:4.2.5.0"])
  ...
  if (is_small_callset) {
    ...
    if (is_gp2_only) {
      ...
      call CollectVariantCallingMetrics as CollectMetricsOnGP2Vcf {
        input:
  ...
  output {
    ...
    File? output_vcf = select_first([Subset_vcf.output_vcf, FinalGatherVcf.output_vcf])
    File? output_vcf_index = select_first([Subset_vcf.output_vcf_index, FinalGatherVcf.output_vcf_index])

    # select metrics from the small callset (gp2_only or all) or from large callset
    File? detail_metrics_file = if is_small_callset then select_first([CollectMetricsOnGP2Vcf.detail_metrics_file ,CollectMetricsOnFullVcf.detail_metrics_file]) else GatherMetrics.detail_metrics_file
    File? summary_metrics_file = if is_small_callset then select_first([CollectMetricsOnGP2Vcf.summary_metrics_file ,CollectMetricsOnFullVcf.summary_metrics_file]) else GatherMetrics.summary_metrics_file
