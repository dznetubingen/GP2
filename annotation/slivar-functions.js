var config = {min_GQ: 20, min_AB: 0.20, min_DP: 5}
// hi quality variants
...
// function for finding het common in affected only 
function find_het(aff, unaff) {
    for(i=0;i<aff.length;i++){
        if(aff[i].alts != 1 || !(hq1(s))) { return false; }
    }
...
// function for finding homalt common in affected only

function find_homalt(aff, unaff) {
    for(i=0;i<aff.length;i++){
        if(aff[i].alts != 2 || !(hq1(s))) { return false; }
    }
    for(i=0;i<unaff.length;i++){
        if(unaff[i].alts != 0) { return false; }
    }
